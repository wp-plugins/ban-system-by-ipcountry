...
if(isset($_SERVER['HTTP_REFERER'])) {$site = parse_url($_SERVER['HTTP_REFERER']); $site = $site['host'];}else{$site="Dirrect";}
...
if(strtolower($co) == strtolower($country)){$c=1; $cc="country";}
}}
if($c == 0){
   ...
   for($i=0; $i<=$m; $i++){
$dtnow = new DateTime(date('m/d/Y H:i'));   $tm =  $get->Ban[$i]->end;   $dtthan = new DateTime($tm);    if($dtnow > $dtthan){$expired = "TRUE";}else {$expired = "FALSE";} 
       $ipg =  $get->Ban[$i]->ip;
       if($ip==$ipg&&$expired=="FALSE"){$found=$i;$c = 1;$cc="dir";}
       if($cookie=="1"){
       if(isset($_COOKIE['ban'])&&$found=='a'){
       $ip_xml = $get->Ban[$i]->ip;
       if($ip_xml == $_COOKIE['ban']&&$expired=="FALSE"){$c = 1; $found=$i;}
       }}
       else{setcookie("ban", "", time()-3600);}
       if($c == 0){setcookie("ban", "", time()-3600);}
   }
   //adding cookie if necessary
   if(!isset($_COOKIE['ban'])&&$cc=="dir"&&$cookie=="1"){$dtnow = new DateTime(date('m/d/Y H:i'));   $tm =  $get->Ban[$found]->end;   $dtthan = new DateTime($tm);    if($dtnow > $dtthan){$expired = TRUE;}else {$expired = FALSE;}    $name =  $get->Ban[$found]->ip;    if($ip==$name&&$expired==FALSE){$c=1;}    if(!isset($_COOKIE['ban'])&&$c>0){$diff = abs(strtotime(date('m/d/Y H:i')) - strtotime($tm)); setcookie("ban", $ip, time() +$diff);}}
   //finished checking
}
if($c>0){$ban = 'Banned';}else{$ban = 'Not Banned';}
if($cc=="country"){$ban = 'Banned by country';}
$fullc = country_code_to_country($country);
$d = date("ymd");
$dy = date("ymd", time() - 86400);
if(is_admin()) goto skipstat;
...
if(is_admin()&&get_option("wp-admin")==0) goto end;
if(is_login_page()&&get_option("wp-login")==0) goto end;
$or = $get->Ban[$found]->or; if($or=="redir"){Header('Location: '.$get->Ban[$found]->redirurl);}else{$reason = $get->Ban[$found]->reason;if($reason==""){$reason="None";}}
$txt = get_option("b");
